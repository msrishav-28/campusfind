-- CampusFind PostgreSQL / Supabase Production Schema
-- Designed for complete multi-tenant partition isolation and serverless persistence.
...
-- 8. Row Level Security (RLS) Configurations
...
-- Public can read approved campuses
create policy "Public can view approved campuses"
  on campuses for select
  using (status = 'approved');

-- Service role has full unrestricted access for all operations
...
-- Public view sanitizing private items columns (secret_hash, poster_session_id, poster_user_id)
