...
function seedItems(existing: ItemRecord[]): ItemRecord[] {
  ...
  const now = nowIso();
  const make = (type: "lost" | "found", title: string, placeId: string, category: (typeof ITEM_CATEGORIES)[number]) => {
  ...
  ].filter((v): v is ItemRecord => Boolean(v));
}

function sanitizeItem(item: ItemRecord): Omit<ItemRecord, "secretHash" | "posterSessionId" | "posterUserId"> {
  const { secretHash: _, posterSessionId: __, posterUserId: ___, ...rest } = item;
  return rest;
}
